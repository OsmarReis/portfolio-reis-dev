    ...
    <div className="max-w-[450px] flex gap-7">
      <div className="flex flex-col gap-10 justify-center items-center pb-2">
        <div className="p-2 bg-yellow-400 border border-black rounded-full flex items-center justify-center">
              <BusinessCenterIcon className="size-7"/>
        </div>
      ...
      <div className="flex flex-col gap-8">
        <h1 className="text-2xl px-1">Experience</h1>
        {education_list.map((item, index) => {
          return(
            <ExperienceContent key={index} title={item.title} from_year={item.from_year} to_year={item.to_year} institution={item.institution} description={item.description} />
          )
        })}
      </div>
